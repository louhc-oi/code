...
templatetypename T inline void cmax( T &x, T y ){ x  y  x = y  x; }
templatetypename T inline void cmin( T &x, T y ){ y  x  x = y  x; }
...
namespace nwf{
	...
	queueint Q; int d[MAXE], l[MAXE], res; bool vis[MAXE];
...
int main(){
	...
	printf( %dn, nwfcalc() );
	t_ed = clock();
	fprintf( stderr, n========info========ntime  %.3fn====================n, (double)( t_ed - t_bg )  CLOCKS_PER_SEC );
	return 0;
}
